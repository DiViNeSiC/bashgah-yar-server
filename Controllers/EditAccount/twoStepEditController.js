...
const resetPasswordConfirm = async (req, res) => {
    ...
    try {
        const payload = await jwt.verify(resetPassToken, process.env.JWT_RESET_PASS_SECRET)
        if (!payload) 
...
const deleteAccountConfirm = async (req, res) => {
    ...
    try {
        const payload = await jwt.verify(deleteAccountToken, process.env.JWT_DELETE_ACC_SECRET)
        if (!payload) 
